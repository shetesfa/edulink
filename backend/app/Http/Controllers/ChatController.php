...
class ChatController extends Controller
{
    ...
    public function sendPrivate(Request $request, int $userId): JsonResponse
    {
        ...
        if (!$request->filled('body') && !$request->filled('file_id')) {
            return response()->json(['success' => false, 'message' => 'Message body or file is required.'], 422);
        }

        if ($request->filled('reply_to_id')) {
            $reply = Message::findOrFail($request->reply_to_id);
            if (!$this->canAccessMessage($reply, $sender)) {
                return response()->json(['success' => false, 'message' => 'Invalid reply target.'], 403);
            }
        }
        ...
        if ($request->file_id) {
            File::where('id', $request->file_id)
                ...
                ->update(['related_type' => 'message', 'related_id' => $message->id]);
        }
        ...
        Notification::create([
            ...
            'body'         => substr($message->body, 0, 100) . (strlen($message->body) > 100 ? '...' : ''),
            'icon'         => 'MessageSquare',
            'related_type' => 'message',
            'related_id'   => $message->id,
            'action_url'   => "/chat/private/{$sender->id}",
            'is_read'      => 0,
            'created_at'   => now(),
        ]);
    ...
    public function sendGroup(Request $request, int $groupId): JsonResponse
    {
        ...
        if ($request->filled('reply_to_id')) {
            $reply = Message::findOrFail($request->reply_to_id);
            if ((int) $reply->group_id !== $groupId || !$this->canAccessMessage($reply, $user)) {
                return response()->json(['success' => false, 'message' => 'Invalid reply target.'], 403);
            }
        }
        ...
        $members = GroupChatMember::where('group_id', $groupId)
            ->where('user_id', '!=', $user->id)
            ->pluck('user_id');
        
        $notifications = $members->map(fn($id) => [
            'user_id'      => $id,
            'type'         => 'new_message',
    ...
    public function addMember(Request $request, int $groupId): JsonResponse
    {
        ...
        $actor = GroupChatMember::where('group_id', $groupId)
            ->where('user_id', $request->user()->id)
            ->firstOrFail();
        if (!in_array($actor->role, ['admin', 'owner'], true)) {
    ...
    {
        $message = Message::findOrFail($id);
        if (!$this->canManageMessage($message, $request->user())) {
            return response()->json(['success' => false, 'message' => 'Unauthorized'], 403);
        }
    ...
    public function forwardMessage(Request $request, int $id): JsonResponse
    {
        ...
        if (!$this->canAccessMessage($original, $user)) {
            return response()->json(['success' => false, 'message' => 'Unauthorized'], 403);
        }

        if (!$request->filled('receiver_id') && !$request->filled('group_id')) {
            return response()->json(['success' => false, 'message' => 'Forward target is required.'], 422);
        }

        if ($request->filled('group_id')) {
            $isMember = GroupChatMember::where('group_id', $request->group_id)->where('user_id', $user->id)->exists();
            if (!$isMember) {
    ...
    private function canAccessMessage(Message $message, User $user): bool
    {
        if ($message->group_id) {
    ...
    private function canManageMessage(Message $message, User $user): bool
    {
        if (!$this->canAccessMessage($message, $user)) {
            return false;
        }
