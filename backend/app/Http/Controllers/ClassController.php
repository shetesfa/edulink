...
class ClassController extends Controller
{
    ...
    {
        $user = $request->user();
        $this->authorizeClassManagement($user);
    ...
    public function join(Request $request): JsonResponse
    {
        ...
        if ($user->role->name === 'teacher') {
            if ($class->teacher_id === $user->id) {
                return response()->json(['success' => false, 'message' => 'You are already the teacher of this class.'], 409);
            }
            
            $class->update(['teacher_id' => $user->id]);
            
            // Add to class group chat as admin
            $group = GroupChat::where('class_id', $class->id)->first();
            if ($group) {
                GroupChatMember::updateOrCreate(
            ...
            return response()->json([
                'success' => true,
                'message' => "You have joined {$class->name} as the teacher!",
                'class'   => $this->formatClass($class->fresh(['teacher', 'grade']), $user->id),
            ]);
        }
    ...
    public function joinByLink(Request $request, string $link): JsonResponse
    {
        $request->merge(['join_code' => $link]);
    ...
    {
        $class = Classes::findOrFail($classId);
        if (!$this->canManageClass($class, $request->user())) {
            return response()->json(['success' => false, 'message' => 'Unauthorized'], 403);
        }
    ...
    {
        if ($user->role->name === 'school_admin' && $class->school_id === $user->school_id) return;
    ...
    private function authorizeClassManagement($user): void
    {
