...
class LessonController extends Controller
{
    ...
    public function show(Request $request, int $classId, int $id): JsonResponse
    {
        ...
        if ($user->role->name === 'student' && !$lesson->is_published) {
            return response()->json(['success' => false, 'message' => 'Not found'], 404);
        }
    ...
    public function toggleBookmark(Request $request, int $classId, int $id): JsonResponse
    {
        ...
        $this->authorizeClassAccess(Classes::findOrFail($classId), $user);
    ...
    public function addComment(Request $request, int $classId, int $id): JsonResponse
    {
        ...
        $this->authorizeClassAccess(Classes::findOrFail($classId), $request->user());
    ...
    private function authorizeClassAccess(Classes $class, $user): void
    {
        if ($class->teacher_id === $user->id) {
            return;
        }

        if ($user->role->name === 'school_admin' && $class->school_id === $user->school_id) {
            return;
        }

        if (Enrollment::where('class_id', $class->id)->where('student_id', $user->id)->exists()) {
            return;
        }
