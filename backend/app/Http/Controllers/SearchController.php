...
class SearchController extends Controller
{
    public function search(Request $request): JsonResponse
    {
        ...
        // User browse / free messaging: allow empty q
        if ($type === 'users' && strlen(trim((string) $request->q)) < 2) {
            $request->merge(['q' => '']);
        } elseif ($type !== 'users') {
        ...
        if (in_array($type, ['all','users'])) {
            $roleFilter = $request->get('role'); // student | teacher | null
            $roleIds = match ($roleFilter) {
                'student' => [2],
                'teacher' => [3, 4], // 4 = legacy school_admin → treat as teacher
                default   => [2, 3, 4],
            };
            ...
            if (strlen(trim($request->q)) >= 2) {
                $userQuery->where(function ($sq) use ($q) {
