...
class VideoMeetingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $meetings = VideoMeeting::where('host_id', $user->id)
            ...
            ->when($user->role->name === 'school_admin', fn($q) => $q->orWhereHas('class', fn($classQuery) => $classQuery->where('school_id', $user->school_id)))
            ->with(['host', 'class'])
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        if ($request->class_id) {
            ...
            if (!$this->canManageClass($class, $user)) {
                return response()->json(['success' => false, 'message' => 'Unauthorized class.'], 403);
            }
        }
        ...
        if ($request->class_id) {
            $class = $class ?? Classes::findOrFail($request->class_id);
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$this->canAccessMeeting($meeting, $request->user())) {
    ...
    private function canAccessMeeting(VideoMeeting $meeting, $user): bool
    {
        if ($meeting->host_id === $user->id) {
            return true;
        }
        ...
        $class = $meeting->class ?: Classes::find($meeting->class_id);

        return $class && (
            $this->canManageClass($class, $user)
            || Enrollment::where('class_id', $meeting->class_id)->where('student_id', $user->id)->exists()
        );
    }

    private function canManageClass(Classes $class, $user): bool
    {
        return $class->teacher_id === $user->id
            || ($user->role()->value('name') === 'school_admin' && $class->school_id === $user->school_id);
    }
}
