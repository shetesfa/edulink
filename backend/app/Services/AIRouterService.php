...
class AIRouterService
{
    ...
    public function ask(string $prompt, array $options = []): array
    {
        ...
        foreach ($this->providers as $providerKey => $config) {
            ...
            try {
                ...
                if ($response) {
                    // Track usage (non-critical - don't let this crash the response)
                    try {
                        $this->trackUsage($providerKey);
                    } catch (\Exception $e) {
                        Log::warning("AIRouter: usage tracking failed (non-critical): " . $e->getMessage());
                    }
                ...
                Log::warning("AIRouter: {$providerKey} returned empty response, trying next.");
            } catch (\Exception $e) {
    ...
    private function callGemini(array $messages, string $system): ?string
    {
        ...
        $data = $response->json();
        if (isset($data['error'])) {
            Log::warning("AIRouter: gemini returned error: " . json_encode($data['error']));
            return null;
        }
    ...
    private function callGroq(array $messages, string $system): ?string
    {
        ...
        $data = $response->json();
        if (isset($data['error'])) {
            Log::warning("AIRouter: groq returned error: " . json_encode($data['error']));
            return null;
        }
...
Include estimated completion time at the top.";
    ...
    private function trackUsage(string $provider): void
    {
        try {
            ...
            Cache::put($cacheKey, $count, now()->endOfDay()->addHour());
        } catch (\Exception $e) {
            // Non-critical — never crash a successful AI response
        }
    }
