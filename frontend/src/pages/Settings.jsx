...
export default function Settings() {
  ...
  const applyThemeColor = (theme) => {
    ...
    const palette = colorPalettes[theme] || colorPalettes.purple;
    Object.keys(palette).forEach(key => {
      document.documentElement.style.setProperty(`--brand-${key}`, palette[key]);
      console.log(`Set --brand-${key} to`, palette[key]);
    });
  };
  ...
  const changePassword = async (e) => {
    ...
    setPwSaving(true);
    try {
      const payload = user?.password_set !== false
        ? pwForm
        : { password: pwForm.password, password_confirmation: pwForm.password_confirmation };
      const { data } = await authAPI.changePassword(payload);
      toast.success(user?.password_set === false ? 'Password set successfully!' : 'Password changed successfully!');
                ...
                <div className="p-5">
                  ...
                  <form onSubmit={changePassword} className="space-y-4">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                      {user?.password_set !== false ? 'Change Password' : 'Set Password'}
                    </h3>
                    ...
                    <button type="submit" disabled={pwSaving || (user?.password_set !== false && !pwForm.current_password) || !pwForm.password}
                      ...
                      {pwSaving ? 'Saving…' : (user?.password_set !== false ? 'Change Password' : 'Set Password')}
                    </button>
                  </form>
