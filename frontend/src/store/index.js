...
export const useAuthStore = create(
  persist(
    (set, get) => ({
      ...
      themeColor:   () => get().user?.settings?.theme ?? 'purple',
      fontSize:     () => get().user?.settings?.font_size ?? 'medium',
      ...
      connectSocket: () => {
        ...
        const s = io(import.meta.env.VITE_SOCKET_URL || 'https://edulink-backend-jxd2.onrender.com', {
          auth: { token },
